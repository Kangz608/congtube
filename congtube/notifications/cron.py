...
def order_confirmations():
    ...
    today_orders = Order.objects.filter(
        Q(created_at__day=day) & Q(status=2)).select_related('channel').values('channel__name','channel__slug').order_by('channel').distinct()
    for today_order in today_orders:
        name = today_order['channel__name']
        slug = today_order['channel__slug']
